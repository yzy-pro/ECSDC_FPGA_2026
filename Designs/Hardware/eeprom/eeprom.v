...
module eeprom #(
    ...
    // POP 与 LOAD 分开，给同步 FIFO 一个完整时钟周期稳定输出 rd_data。
    assign rx_fifo_rd_en = (ctrl_state == CTRL_POP);

    // 只有读事务正常完成且 TX FIFO 未满时，才把结果写入 TX FIFO。
    ...
    // 第一段状态机逻辑：根据当前状态、请求和 FIFO 状态计算下一状态。
    always @(*) begin
